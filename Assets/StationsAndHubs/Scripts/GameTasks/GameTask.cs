...
namespace StationsAndHubs.Scripts.GameTasks
{
    ...
    public GameTask(string id, string name, string location, string next) {
        this.id=id;
        this.name=name;
    ...
    public static void LoadGameTasks(string taskFileName)
    {
        ...
        try {
            sc = new StreamReader(Initializer.workingDirectory+"/"+Initializer.taskFilesDir+"/"+taskFileName);
    ...
    private static GameTask parseGameTaskFromLine(string line) {
        line = line.Replace(", ",",");
        Debug.Log(line);
